...
export class LocationService {
  ...
  private readonly CANELONES_ZONES = [
    ...
    "Paso Carrasco", "Colonia Nicolich", "Aeropuerto", "Canelones",
  ...
  private detectInitialLocation() {
    // 1. PRIORIDAD MÁXIMA: Inyección desde Cloudflare Worker
    // Esto significa que el servidor detectó la IP fresca en esta carga.
    // Sobrescribe el storage antiguo porque es la ubicación "real" actual.
    const injectedLoc = (window as any).SC_INJECTED_LOC;
  ...
  private setCleanLocation(rawLocation: string) {
    ...
    if (clean.length > 2) { // Evitar strings vacíos o basura corta
